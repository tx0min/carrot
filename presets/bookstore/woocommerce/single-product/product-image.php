...
$full_size_image   = wp_get_attachment_image_src( $post_thumbnail_id, 'shop_catalog' );
...
$wrapper_classes   = apply_filters( 'woocommerce_single_product_image_gallery_classes', array(
	...
	'images',
		...
		$data=array(
			...
			"synopsys"=> trim_text(get_the_excerpt(),300),
			"show-controller"=> false,
			"show-simple-controller"=> true,
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
		...
		if ( has_post_thumbnail() ) {
			$html  = '<div data-thumb="' . get_the_post_thumbnail_url( $post->ID, 'shop_catalog' ) . '" class="woocommerce-product-gallery__image">';
			//$html .= get_the_post_thumbnail( $post->ID, 'shop_catalog', $attributes );
			$html .= get_post_thumbnail_data( 'shop_catalog', $data );
			$html .= '</div>';
		} else {
